...
*** Compute
   ...
   :END:
    ...
    #+begin_src f90 :comments org :tangle (eval f) :noweb yes
integer function qmckl_compute_factor_en_deriv_e_doc_f( &
     context, walk_num, elec_num, nucl_num, type_nucl_num, &
     type_nucl_vector, aord_num, aord_vector, &
...
end function qmckl_compute_factor_en_deriv_e_doc_f
    #+end_src
    
#+begin_src c :tangle (eval c) :comments org :exports none
    qmckl_exit_code qmckl_compute_factor_en_deriv_e_hpc (
          const qmckl_context context,
          const int64_t walk_num,
          const int64_t elec_num,
          const int64_t nucl_num,
  ...
  for (int kk=0; kk<elec_num*4*walk_num; ++kk){
    factor_en_deriv_e[kk] = 0.0;
  }
  third = 1.0 / 3.0;
  ...
  for (int nw=0; nw < walk_num; ++nw) {
    for (int a=0; a < nucl_num ; ++a) {
      for (int i=0; i < elec_num; ++i) {
        ...
        printf("%d\t%d\n", a, type_nucl_vector[a]);
        den = 1.0 + aord_vector[1+(type_nucl_vector[a]-1)*(aord_num+1)] * x;
        invden = 1.0 / den;
        invden2 = invden * invden;
        invden3 = invden2 * invden;
        xinv = 1.0 / x;
        ...
        for (int ii = 0; ii < 4; ++ii) {
          dx[ii] = en_distance_rescaled_deriv_e[ii+i*4+a*4*elec_num+nw*4*elec_num*nucl_num];
        }
        ...
        for (int ii = 0; ii < 3; ++ii){
          ...
          for (int p=1; p < aord_num; ++p){
            y = p * aord_vector[p + 1 + (type_nucl_vector[a]-1)*(aord_num+1)] * x;
            power_ser_g[ii] = power_ser_g[ii] + y * dx[ii];
            lap1 = lap1 + (p - 1) * y * xinv * dx[ii] * dx[ii];
            lap2 = lap2 + y;
            x = x * en_distance_rescaled[i+a*elec_num+nw*elec_num*nucl_num];
          }

          lap3 = lap3 - 2.0 * aord_vector[p + 1 + (type_nucl_vector[a]-1)*(aord_num+1)] * dx[ii] * dx[ii];

          factor_en_deriv_e[i+ii*elec_num+nw*elec_num*4] = factor_en_deriv_e[i+ii*elec_num+nw*elec_num*4] \
                                                         + aord_vector[type_nucl_vector[a]*(aord_num+1)] * dx[ii] * invden2  \
                                                         + power_ser_g[ii];
        ...
        lap2 = lap2 * dx[ii] * third;
        lap3 = lap3 + den * dx[ii];
        lap3 = lap3 * aord_vector[(type_nucl_vector[a]-1)*(aord_num+1)] * invden3;
        factor_en_deriv_e[i+ii*elec_num+nw*elec_num*4] += lap1 + lap2 + lap3;
...
#   #+CALL: generate_c_header(table=qmckl_factor_en_deriv_e_args,rettyp=get_value("CRetType"),fname=get_value("Name"))
    ...
    #+begin_src c :comments org :tangle (eval h_private_func) :noweb yes :exports none
    qmckl_exit_code qmckl_compute_factor_en_deriv_e_doc (
          const qmckl_context context,
          const int64_t walk_num,
          const int64_t elec_num,
          const int64_t nucl_num,
    ...
    #+CALL: generate_c_interface(table=qmckl_factor_en_deriv_e_args,rettyp=get_value("CRetType"),fname="qmckl_compute_factor_en_deriv_e_doc")

    #+RESULTS:
    #+begin_src f90 :tangle (eval f) :comments org :exports none
    integer(c_int32_t) function qmckl_compute_factor_en_deriv_e_doc &
        (context, &
         walk_num, &
         elec_num, &
         nucl_num, &
      ...
      integer(c_int32_t), external :: qmckl_compute_factor_en_deriv_e_doc_f
      info = qmckl_compute_factor_en_deriv_e_doc_f &
             (context, &
         walk_num, &
         elec_num, &
         nucl_num, &
    ...
    end function qmckl_compute_factor_en_deriv_e_doc
    #+end_src
  ...
  #ifdef HAVE_HPC
  return qmckl_compute_factor_en_deriv_e_hpc(context, walk_num, elec_num, nucl_num,type_nucl_num, type_nucl_vector, aord_num, aord_vector, en_distance_rescaled, en_distance_rescaled_deriv_e, factor_en_deriv_e);
  #else
  return qmckl_compute_factor_en_deriv_e_doc(context, walk_num, elec_num, nucl_num,type_nucl_num, type_nucl_vector, aord_num, aord_vector, en_distance_rescaled, en_distance_rescaled_deriv_e, factor_en_deriv_e);
  #endif
}
    #+end_src
