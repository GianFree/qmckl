  ...
  for (int nw = 0; nw < walk_num; ++nw) {
    for (int ii = 0; ii < 4; ++ii) {
      for (int j = 0; j < elec_num; ++j) {
        factor_ee_deriv_e[j * (walk_num * 4) + ii * (walk_num) + nw]  = 0.0;
      }
    }
  }
    ...
    for (int i = 0; i < elec_num; ++i) {
      for (int j = 0; j < elec_num; ++j) {
        ...
        dx[0] = ee_distance_rescaled_deriv_e[0 \
                                           + i * (walk_num * elec_num) \
                                           + j * elec_num + nw];
        dx[1] = ee_distance_rescaled_deriv_e[1 * (walk_num * elec_num * elec_num) \
                                           + i * (walk_num * elec_num) \
                                           + j * elec_num + nw];
        ...
        for (int ii = 0; ii < 3; ++ii) {
          ...
          for (int p = 1; p < bord_num; ++p) {
            ...
            lap1 = lap1 + (p - 1) * y * xinv * dx[ii] * dx[ii];
            lap2 = lap2 + y;
            x = x * ee_distance_rescaled[j * (walk_num * elec_num) + i * (walk_num) + nw];
          }

          lap3 = lap3 - 2.0 * bord_vector[1] * dx[ii] * dx[ii];

          factor_ee_deriv_e[j * (walk_num * 4) + ii * (walk_num) + nw] +=              \
        ...
        factor_ee_deriv_e[j * (walk_num * 4) + ii * (walk_num) + nw] += + lap1 + lap2 + lap3;
...
// value of (0,2,1)
printf("%.19f", een_rescaled_e[0][1][0][2]);
...
qmckl_exit_code qmckl_compute_cord_vect_full (
  ...
  for (int a=0; a < nucl_num; ++a){
    for (int i=0; i < dim_cord_vect; ++i) {
      cord_vect_full[i+a*dim_cord_vect] = cord_vector[i+(type_nucl_vector[a]-1)*dim_cord_vect];
    }
  }
...
printf("bubu %.19f", dtmp_c[0][1][0][0][0][0]);
...
assert(fabs(factor_een[0] + 0.37407972141304213) < 1e-10);
     #+end_src
