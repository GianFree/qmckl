...
//  
  ...
  double   x;
  int      i, j, k, l, nw, len_een_ij; 
  
  // number of element for the een_rescaled_e_ij[N_e*(N_e-1)/2][cord+1]
  // probably in C is better [cord+1, Ne*(Ne-1)/2]
  len_een_ij = elec_num * (elec_num - 1) / 2 * (cord_num + 1);
  een_rescaled_e_ij = (double *) malloc(sizeof(double) * len_een_ij);
    ...
    for (int l = 0; l < (cord_num + 1); ++l) {
      for (int i = 0; i < elec_num; ++i) {
        for (int j = 0; j < elec_num; ++j) {
          een_rescaled_e[j + i*elec_num + l*elec_num*elec_num + nw*(cord_num+1)*elec_num*elec_num]
        }
      }
    }
  }
  ...
  for (int nw = 0; nw < walk_num; ++nw) {
    ...
    for (int kk = 0; kk < len_een_ij; ++kk) {
      ...
      een_rescaled_e_ij[kk]= ( kk < (elec_num*(elec_num-1)/2) ? 1.0 : 0.0 );
    }
    ...
    do i = 1, j - 1
      k = k + 1;
      // een_rescaled_e_ij(k, 2) = dexp(-rescale_factor_kappa_ee * ee_distance(i, j, nw));
      een_rescaled_e_ij[k + elec_num*(elec_num-1)/2] = exp(-rescale_factor_kappa_ee * \
                                                            ee_distance[i, j, nw]);
      }
    }
